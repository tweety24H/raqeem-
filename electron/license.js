...
// يزيل حماية كلمة السر عن تبويب الترخيص/النسخ الاحتياطي بالكامل — المالك
// يقدر يعيّن وحدة جديدة لاحقًا من نفس التبويب إذا أراد.
function removeAdminPassword() {
  store.delete('adminPasswordHash');
}
...
// رقم واتساب المالك الشخصي (وليس رقم شركة رقيم) — يظهر بصفحة التفعيل حتى
// الزبون يتواصل مباشرة مع صاحب المطبعة نفسه لطلب مفتاح تفعيل، بدل رقم عام
// ثابت بالكود. نطبّع الصيغة دائمًا لدولي بدون + (964xxxxxxxxxx) بغض النظر
// كيف كتبه المالك (مع +، أو برقم محلي يبدأ بصفر، أو حتى صفر زائد بعد كود
// الدولة لو نسخه بالغلط) حتى رابط wa.me يشتغل دائمًا.
function normalizeWhatsAppNumber(raw) {
  let digits = String(raw || '').replace(/\D/g, '');
  if (!digits) return '';
  if (digits.startsWith('964')) {
    const rest = digits.slice(3);
    digits = '964' + (rest.startsWith('0') ? rest.slice(1) : rest);
...
function getOwnerWhatsAppNumber() {
  return store.get('ownerWhatsAppNumber') || '';
}

function setOwnerWhatsAppNumber(number) {
  const normalized = normalizeWhatsAppNumber(number);
  store.set('ownerWhatsAppNumber', normalized);
  return normalized;
}
