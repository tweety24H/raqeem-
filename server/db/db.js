...
// ترقية: أعمدة إضافية لجدول customer_requests لدعم QR الطلب الكامل (نوع
// الطلب، الكمية، المواصفات، الخدمة المختارة) وسبب الرفض لمودال "رفض" الجديد.
// request_type: 'design_upload' (QR البسيط الحالي - رفع تصميم فقط) أو
// 'full_order' (QR الجديد - فورم كامل بدون تسجيل دخول).
function migrateCustomerRequestsColumns() {
  const existing = db.prepare('PRAGMA table_info(customer_requests)').all().map((c) => c.name);
  const wanted = [
    ['request_type', "TEXT NOT NULL DEFAULT 'design_upload'"],
  ...
  for (const [col, def] of wanted) {
    if (!existing.includes(col)) {
      db.exec(`ALTER TABLE customer_requests ADD COLUMN ${col} ${def}`);
    }
  }
}
migrateCustomerRequestsColumns();
...
function seedIfEmpty() {
  ...
  if (settingsCount === 0) {
    ...
    const defaults = {
      ...
      currency: 'IQD',
      whatsapp_enabled: '0',
      whatsapp_message_template: WHATSAPP_TEMPLATE_DEFAULT,
      whatsapp_phone: '',
    ...
    // ترقية: قاعدة بيانات موجودة مسبقًا (أُنشئت قبل إضافة نص رسالة الواتساب
    // القابل للتعديل ورقم الإرسال) ما تملك هذين المفتاحين — نضيفهم هنا بقيم
    // افتراضية. لازم تبقى داخل هذا الفرع (settingsCount > 0) وليس قبل
    // seedIfEmpty، وإلا هذا الـ INSERT نفسه يخلي فحص "القاعدة فاضية" أعلاه
    // يفشل على أول تشغيل فيفوّت زرع بقية الإعدادات الافتراضية بالكامل.
    const insertIfMissing = db.prepare('INSERT OR IGNORE INTO settings (key, value) VALUES (?, ?)');
    insertIfMissing.run('whatsapp_message_template', WHATSAPP_TEMPLATE_DEFAULT);
    insertIfMissing.run('whatsapp_phone', '');
  }
}
