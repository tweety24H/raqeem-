...
// يبني نص الرسالة من القالب المحفوظ بالإعدادات (أو الافتراضي إذا فاضي)
// ويستبدل المتغيرات {اسم_الزبون}/{رقم_الطلب}/{اسم_المطبعة} بقيمها الحقيقية.
// مصدّرة أيضًا لأن الفرونت يعرض نفس هذا الاستبدال كمعاينة قبل الحفظ.
function buildReadyMessage(order) {
  const template = settingsService.get('whatsapp_message_template') || DEFAULT_READY_TEMPLATE;
  const shopName = settingsService.get('shop_name', 'المطبعة');
  return template
    .split('{اسم_الزبون}').join(order.customer_name || '')
    .split('{رقم_الطلب}').join(order.order_number || '')
    .split('{اسم_المطبعة}').join(shopName || '');
}
...
module.exports = { init, getStatus, notifyReady, disconnect, buildReadyMessage };
