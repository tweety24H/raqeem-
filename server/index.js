...
// أداة توليد مفاتيح تفعيل للمطوّرة فقط (صفحة LicenseGenerator.jsx بمتصفح
// ويب عادي، مو Electron) - نفس المسار المستخدم من IPC license:generate.
// حماية مزدوجة: (1) يرفض أي طلب مو من نفس الجهاز (localhost)، (2) حتى لو
// تجاوزها أحد، require أدناه يفشل بأي حال لأن مجلد licensing-vendor/ غير
// موجود إطلاقًا بنسخة الزبون (مستثنى من "files" بـ package.json).
app.post('/api/dev/generate-license', (req, res) => {
  const ip = req.ip || req.socket?.remoteAddress || '';
  const isLocal = ip === '127.0.0.1' || ip === '::1' || ip === '::ffff:127.0.0.1';
  if (!isLocal) return res.status(403).json({ ok: false, error: 'هذه الأداة تعمل فقط من نفس جهاز المطوّرة' });
  try {
    const { signLicense, expFromDuration } = require('../licensing-vendor/sign');
    const { hwid, shop, duration } = req.body || {};
