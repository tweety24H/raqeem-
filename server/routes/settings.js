...
router.get('/network', requireAuth, async (req, res) => {
  ...
  // QR الطلب الكامل (فورم بدون تسجيل دخول: خدمة/كمية/مواصفات/تصميم اختياري)
  // — صفحة HTML ثابتة يخدمها السيرفر نفسه على شبكة الواي فاي المحلية، بعكس
  // صفحات React اللي تفتح فقط داخل نافذة Electron (file://) وما توصلها أي
  // موبايل زبون خارجي.
  const fullOrderFormUrl = `http://${lanIp}:${port}/portal/new-order-form.html`;
  const [customerQr, ownerQr, fullOrderFormQr] = await Promise.all([
    QRCode.toDataURL(customerPortalUrl, { margin: 1, width: 200 }),
    QRCode.toDataURL(ownerPortalUrl, { margin: 1, width: 200 }),
    QRCode.toDataURL(fullOrderFormUrl, { margin: 1, width: 200 }),
  ]);
  res.json({ lanIp, port, customerPortalUrl, ownerPortalUrl, customerQr, ownerQr, fullOrderFormUrl, fullOrderFormQr });
});
