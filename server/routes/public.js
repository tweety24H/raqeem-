...
// POST /api/public/full-order-request (multipart) — QR الثاني: فورم طلب
// كامل بدون تسجيل دخول (خدمة/كمية/مواصفات + تصميم اختياري)، بعكس
// /order-request البسيط أعلاه (رفع تصميم فقط). يُخزَّن بنفس جدول
// customer_requests (نفس صفحة "طلبات الزبائن الواردة") مميزًا بـ
// request_type='full_order' حتى تظهر عليه علامة "طلب كامل" بالواجهة.
router.post('/full-order-request', upload.single('file'), (req, res) => {
  const { customer_name, phone, service_id, quantity, specs, notes } = req.body;
  if (!customer_name || !phone) {
    return res.status(400).json({ error: 'الاسم ورقم الهاتف مطلوبان' });
  }
  const service = service_id
    ? db.prepare('SELECT name FROM services WHERE id = ? AND active = 1').get(service_id)
    : null;
  const filePath = req.file ? path.join('uploads', 'requests', req.file.filename) : null;
  const info = db
    .prepare(
      `INSERT INTO customer_requests
         (customer_name, phone, service_text, service_id, quantity, specs, notes, file_path, request_type, status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'full_order', 'وارد من QR - بانتظار المراجعة')`
    )
    .run(
      customer_name,
      phone,
