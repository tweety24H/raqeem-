...
import { Home, Package, Boxes, Users, BarChart3, Archive, Clock, Shield, Settings, ChevronLeft, ChevronDown, LogOut, Repeat, Printer, KeyRound, Key } from 'lucide-react';
...
// نفس شرط ظهور صفحة مولد المفاتيح (LicenseGenerator.jsx) - رابط السايدبار
// يظهر بس لما الصفحة نفسها راح تشتغل فعليًا، حتى ما نعرض رابط ميت بنسخة
// الزبون المحزّمة (import.meta.env.DEV تكون false هناك).
const DEV_TOOLS_VISIBLE = !(typeof window !== 'undefined' && window.raqeem?.isElectron) || import.meta.env.DEV;
...
export default function SidebarGlass({ collapsed, onToggleCollapsed, mobileOpen, onCloseMobile, bottomOffset }) {
  ...
  return (
    <>
      ...
      <aside
        ...
        style={{ width }}
        dir="rtl"
        className={`fixed top-0 ${bottomOffset ? 'bottom-11' : 'bottom-0'} right-0 z-50 flex flex-col overflow-hidden bg-[#0B1D3A] text-slate-300 shadow-[0_0_40px_rgba(0,0,0,0.3)] transition-all duration-300 ease-in-out ${
      ...
      >
        ...
        <div className={`border-t border-white/10 p-4 ${!isExpanded ? 'px-2' : ''}`}>
          ...
          {isOwner && DEV_TOOLS_VISIBLE && (
            ...
            >
              <Key className="h-4 w-4 shrink-0" strokeWidth={1.5} />
              {isExpanded && <span>مولد المفاتيح</span>}
            </button>
          )}
          <button
            type="button"
