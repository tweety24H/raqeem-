...
export default function Layout() {
  ...
  const { t } = useLanguage();
  const { status: licenseStatus, isElectron } = useLicense();
  // TrialBanner (App.jsx) هو fixed bottom-0 على كامل الشاشة، فوق كل شي —
  // بدون هذا الحجز يغطي زر تسجيل الخروج بأسفل السايدبار وآخر عنصر بأي
  // صفحة عند التمرير للنهاية. h-11/pb-11/bottom-11 لازم يبقون متطابقين مع
  // ارتفاع TrialBanner نفسه.
  const showTrialBanner = isElectron && licenseStatus?.status === 'trial';
    ...
    <DashboardSummaryProvider>
      <div className="h-screen w-screen overflow-hidden bg-slate-50 dark:bg-slate-950">
        ...
        >
          ...
          <main className={`flex-1 overflow-y-auto ${showTrialBanner ? 'pb-11' : ''}`}>
